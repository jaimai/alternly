...
class TestPostCrud:
    ...
    def test_invalid_kind_rejected(self, client, auth_headers, db_session):
        headers1, user1, _, _, h = premium_family(client, auth_headers, db_session)
        resp = post(client, headers1, h["id"], kind="rumeur")
        assert resp.status_code == 422

    def test_empty_body_rejected(self, client, auth_headers, db_session):
        headers1, user1, _, _, h = premium_family(client, auth_headers, db_session)
        resp = post(client, headers1, h["id"], body="")
        assert resp.status_code == 422

    def test_author_edits_and_deletes(self, client, auth_headers, db_session):
        headers1, user1, headers2, user2, h = premium_family(client, auth_headers, db_session)
...
class TestCompletion:
    def test_complete_and_reopen(self, client, auth_headers, db_session):
        headers1, user1, headers2, user2, h = premium_family(client, auth_headers, db_session)
...
class TestReplies:
    ...
    def test_only_author_deletes_reply(self, client, auth_headers, db_session):
        headers1, user1, headers2, user2, h = premium_family(client, auth_headers, db_session)
        pid = post(client, headers1, h["id"]).json()["id"]
...
class TestCalendarTasks:
    def test_dated_task_appears_in_calendar(self, client, auth_headers, db_session):
        headers1, user1, _, user2, h = premium_family(client, auth_headers, db_session)
        client.put(
    ...
    def test_completed_task_not_in_calendar(self, client, auth_headers, db_session):
        headers1, user1, _, user2, h = premium_family(client, auth_headers, db_session)
        client.put(
            f"/api/households/{h['id']}/custody-rule",
            json={"pattern": "alternate_weeks", "start_date": "2026-01-05", "reference_parent_id": user1["id"]},
